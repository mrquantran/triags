...
def render_set(model_path, name, iteration, views, scene, gaussians, pipeline, background,
               app_model=None, max_depth=5.0, volume=None, use_depth_filter=False, kernel_size=None):
    # box
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        gt, _ = view.get_image()
        out = render(view, gaussians, pipeline, background, kernel_size=kernel_size)
        rendering = out["render"]
        _, H, W = rendering.shape

        depth = out["median_depth"].squeeze()
        ...
        if use_depth_filter:
            ...
            depth_normal = out["normal"].permute(1,2,0)
...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool,
    ...
    with torch.no_grad():
        ...
        sdf_trunc = float(sdf_trunc_scale) * voxel_size
        print(f"voxel_size {voxel_size}, sdf_trunc {sdf_trunc}, max_depth {max_depth}")
        ...
        if not skip_train:
            render_set(dataset.model_path, "train", scene.loaded_iter, scene.getTrainCameras(), scene, gaussians, pipeline, background,
                       max_depth=max_depth, volume=volume, use_depth_filter=use_depth_filter, kernel_size=dataset.kernel_size)
            print(f"extract_triangle_mesh")
            mesh = volume.extract_triangle_mesh()
            path = os.path.join(dataset.model_path, "mesh")
            os.makedirs(path, exist_ok=True)

            o3d.io.write_triangle_mesh(os.path.join(path, f"tsdf_fusion_{sdf_trunc_scale}.ply"), mesh,
                                       write_triangle_uvs=True, write_vertex_colors=True, write_vertex_normals=True)

            mesh = post_process_mesh(mesh, num_cluster)
            o3d.io.write_triangle_mesh(os.path.join(path, f"tsdf_fusion_post_{sdf_trunc_scale}.ply"), mesh,
                                       write_triangle_uvs=True, write_vertex_colors=True, write_vertex_normals=True)

        if not skip_test:
            render_set(dataset.model_path, "test", scene.loaded_iter, scene.getTestCameras(), scene, gaussians, pipeline, background, kernel_size=dataset.kernel_size)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sdf_trunc_scale", default=4, type=float)
