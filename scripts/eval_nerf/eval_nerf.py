...
def compute_chamfer_meshes(gt:trimesh.Trimesh, pred: trimesh.Trimesh, num_points=1000000, norm=2):
    ...
    """
    print('compute chamfer distance')
    ...
    print(f"Accuracy: {result['accuracy']:.4f} cm")
    print(f"Completeness: {result['completeness']:.4f} cm")
    print(f"Chamfer distance: {result['chamfer*100']:.4f} cm")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--resolution', default=400, type=int)
    ...
    if args.evaluate:
        ...
        result = compute_chamfer_meshes(gt_mesh, res)

        chamfer_json_path = mesh_path.parent / f"{mesh_path.stem}.{args.scene}.chamfer_metrics.json"
        with open(chamfer_json_path, 'w') as f:
