...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, kernel_size):
    ...
    paper_path = os.path.join(model_path, name, "ours_{}".format(iteration), "paper")
    makedirs(render_path, exist_ok=True)
    makedirs(gts_path, exist_ok=True)
    makedirs(depth_path, exist_ok=True)
    makedirs(normal_path, exist_ok=True)
    makedirs(paper_path, exist_ok=True)
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        rendered_normal: torch.Tensor = out["normal"]
        ...
        mask = view.mask.squeeze().cpu().numpy()
        mask = (mask > 0.5).astype(np.uint8)  # Ensure binary mask
        normal_mask = normal_show * mask[:, :, np.newaxis]

        cv2.imwrite(os.path.join(paper_path, '{0:05d}'.format(idx) + ".jpg"), normal_mask)

        torchvision.utils.save_image(gt.clamp(0.0, 1.0), os.path.join(gts_path, view.image_name + ".png"))
        torchvision.utils.save_image(rendering, os.path.join(render_path, view.image_name + ".png"))
        cv2.imwrite(os.path.join(depth_path, '{0:05d}'.format(idx) + ".jpg"), depth_color)
        cv2.imwrite(os.path.join(normal_path, '{0:05d}'.format(idx) + ".jpg"), normal_show)
